...
export default function NovelsLibrary() {
  ...
  // Multi-Stage Interface Core State Listeners
  ...
  // Review & Interaction Engagements Threads States
  ...
  // LIVE OMNI-SEARCH EXTRACTOR EVALUATOR
  useEffect(() => {
    let output = novels;
              ...
              <div className="pt-4 border-t border-[#3d3731] bg-[#2b2723] shrink-0 space-y-3 mt-4">
                {currentUser && (
                  <form onSubmit={handleAddComment} className="flex gap-2 items-center">
                    <input type="text" placeholder="Add a public analytical review note..." value={newComment} onChange={(e) => setNewComment(e.target.value)} className="flex-1 bg-[#1e1b19] border border-[#3d3731] text-xs font-sans text-[#ebdcb9] outline-none px-3 py-2.5 rounded-xl placeholder:text-white/5" required />
                ...
                <div className="flex gap-3">
                  <button onClick={(e) => handleLikeToggle(selectedNovel, e)} className={`px-4 bg-[#ebdcb9]/5 border border-[#3d3731] rounded-xl flex items-center justify-center transition-all cursor-pointer ${selectedNovel.likes?.includes(currentUser?.uid) ? "text-red-400 border-red-500/20" : "text-[#c4b28d] hover:text-white"}`}><Heart className={`h-4 w-4 mr-2 ${selectedNovel.likes?.includes(currentUser?.uid) ? "fill-current" : ""}`} /> Like Manuscript</button>
                  <button onClick={() => setActiveReadingNovel(selectedNovel)} className="flex-1 bg-[#ebdcb9] hover:bg-white text-[#24211e] text-[11px] font-sans font-medium uppercase tracking-widest py-3.5 rounded-xl transition-all text-center cursor-pointer shadow-md">Read More</button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-[#2b2723] border border-[#3d3731] rounded-[2rem] w-full max-w-xl p-6 md:p-8 space-y-5 shadow-2xl relative">
            ...
            <form onSubmit={handlePublishNovel} className="space-y-4">
              ...
              <button type="submit" disabled={publishing} className="w-full bg-[#ebdcb9] hover:bg-white text-[#24211e] text-xs font-sans font-medium uppercase tracking-wider py-4 rounded-xl transition-all cursor-pointer shadow-md disabled:opacity-40">{publishing ? "Streaming Layout Packages to Cloud Engine..." : "Publish Novel Catalog"}</button>
            </form>
          </div>
        </div>
      )}
